...
function buildPrintHtml(
  bill: SupplierBill,
  tenant: string,
  opts: { businessName: string; logoBase64: string | null; gstin: string; address: string; phone: string; accentHue: number }
): string {
...
<div class="footer">Goods Received Note &mdash; ${esc(opts.businessName || "Medixor")}</div>
...
export function BillPrintModal({ bill, tenant, onClose }: Props) {
  ...
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(buildPrintHtml(bill, tenant, settings));
    win.document.close();
  };
